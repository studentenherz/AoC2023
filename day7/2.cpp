...
struct Hand{
	...
	Hand(const string& cards, int bid) : cards(cards), bid(bid) {
		...
		static const string labels = "J23456789TQKA";
		for(char c: cards){
...
private:
	Kind get_kind(const string& cards) {
		...
		int max_cnt = 0;
		char max_cnt_label = 0;
		for (auto p : cnt){
			if (p.first != 'J' && p.second > max_cnt) {
				max_cnt = p.second;
				max_cnt_label = p.first;
			}
		}

		if (max_cnt == 0) return FiveOfAKind;

		auto it = cnt.find('J');
		if (it != cnt.end()) {
			cnt[max_cnt_label] += it->second;
			max_cnt += it->second;
			cnt.erase(it);
		}
